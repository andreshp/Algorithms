...
# Depth-First Search for finding the Connected Components of Graph
#######################################################################

# This program read a graph as an adjacency list from a file. 
# Afterwards it executes the DFS algorithm on it to find the number of
...
class Graph(object):
        ...
        if len(self.adj_list) > 0:
            ...
            for node in self.adj_list:
                if node not in visited:
                    visited.add(node)
                    connected_components.append(self.depthFirstSearch(node, visited, []))
    ...
    # Efficiency: O(|Va| + |Ea|)
    def depthFirstSearch(self, a, visited, connected_component):
        for neighbour in self.adj_list[a]:
            if neighbour not in visited:
                visited.add(neighbour)
                connected_component.extend(self.depthFirstSearch(neighbour, visited, []))

        connected_component.append(a)
...
if len(sys.argv) < 2 or len(sys.argv) > 3:
    print("Sintax: DepthFirstSearch.py <options> graph.txt \n The option -n don't print the connected components.")
    sys.exit()
...
# Execute Depth-First Search for connected components and count the time wasted
